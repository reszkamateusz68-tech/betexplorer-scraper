...
target_day = now.date() + timedelta(days=3)
...
print(f"1. Pobieram kalendarz meczów na POPOJUTRZE ({start_iso[:10]})...")
...
print(f"2. Znaleziono {total_events} meczów na popojutrze. Pobieram pełną ofertę rynków (16 wątków)...")

if not total_events:
    print("Brak meczów na popojutrze w ofercie.")
    exit()
...
if all_rows:
    ...
    df_csv.to_csv("superbet_baza_kursow_popojutrze.csv", sep=";", index=False, encoding="utf-8-sig")
    ...
    with open("superbet_baza_popojutrze.json", "w", encoding="utf-8") as f:
        json.dump(superbet_db, f, ensure_ascii=False, indent=2)

    print(f"SUKCES! Baza Superbet (POPOJUTRZE) została zaktualizowana w {elapsed} s.")