...
OPIS: Zaawansowany silnik analityczno-statystyczny integrujący dane meczowe z serwisów
      BetExplorer, SoccerStats oraz Football-Data. Skrypt wykonuje kalibrację matematyczną
      rozkładów Poissona, Skellama i wielowymiarowej kopuły korelacyjnej dla BetBuilder Pro,
      klasyfikuje zespoły do 6 koszyków jakościowych, monitoruje anomalie statystyczne
      (gole, rożne, strzały), mapuje pełną bazę kursów Superbet oraz generuje zróżnicowany
      widok Top_Wybory bez automatycznego zaśmiecania portfela AKO.
====================================================================================================
"""
...
# ==================================================================================================
# 0. KONFIGURACJA ŚRODOWISKA, FLAGI DEWELOPERSKIE I KONTROLA ZASOBÓW
# ==================================================================================================
...
print(f"TRYB TESTOWY (TEST_MODE): {'WŁĄCZONY (Ograniczona liczba lig)' if TEST_MODE else 'WYŁĄCZONY (Pełny skan produkcyjny)'}")
print(f"INTEGRACJA SUPERBET: {'POMINIĘTA (Cache/Tryb szybki)' if SKIP_SUPERBET else 'AKTYWNA (Wczytywanie baz JSON)'}")
print("=" * 90)

# ==================================================================================================
# 1. INICJALIZACJA I OBSŁUGA POŁĄCZENIA Z GOOGLE SHEETS (ODPORNOŚĆ NA BŁĄD 503)
# ==================================================================================================
...
for attempt in range(max_sheets_retries):
    try:
        ...
        print("✅ Pomyślnie połączono z arkuszem Google Sheets: 'BetExplorer'")
        break
    except gspread.exceptions.APIError as e:
        if any(code in str(e) for code in ["503", "429", "500"]) and attempt < max_sheets_retries - 1:
            wait_time = (2 ** attempt) + random.uniform(0.5, 1.5)
            print(f"⚠️ Google Sheets API zwróciło błąd. Oczekiwanie {wait_time:.1f}s przed próbą {attempt + 2}/{max_sheets_retries}...")
...
# Automatyczne usuwanie zbędnych zakładek ze skoroszytu Google Sheets
for sheet_to_remove in ["H2H_Mecze", "Kupon_Specjalisty"]:
    try:
        ...
        print(f"🗑️ Usunięto zbędną zakładkę z Google Sheets: '{sheet_to_remove}'")
    except Exception:
        pass

# ==================================================================================================
# 2. SŁOWNIKI NORMALIZACYJNE NAZW DRUŻYN
# ==================================================================================================
...
try:
    if os.path.exists("slownik_druzyn.json"):
        ...
        print("ℹ️ Brak pliku slownik_druzyn.json. Działanie na oryginalnych nazwach.")
except Exception as err_slownik:
    print(f"⚠️ Ostrzeżenie przy wczytywaniu slownik_druzyn.json: {err_slownik}")

# ==================================================================================================
# 3. GLOBALNE FUNKCJE POMOCNICZE I PARSERY FORMATÓW
# ==================================================================================================

def global_recalc_przedzial(row):
    """Kategoryzuje oszacowany lub realny kurs do odpowiedniego koszyka inwestycyjnego."""
    try:
...
def split_datetime(value):
    """Rozbija format tekstowy daty z BetExplorera na standaryzowaną datę YYYY-MM-DD oraz godzinę."""
    if pd.isna(value):
        return "", ""
...
def categorize_date(d_str):
    """Przypisuje datę meczu do kategorii terminowej dla modułu selekcji."""
...
def get_base_league(league_url_or_name):
    """Czyści URL lub nazwę ligi z parametrów sezonu, zapytań URL i etapów play-off."""
...
def prepare_for_gsheets(df):
    """Normalizuje DataFrame pod kątem formatu liczb i pustych wartości w Google Sheets."""
...
def safe_batch_update(spreadsheet_obj, ws_name, df_data):
    """Bezpieczny zapis danych do arkusza Google Sheets z czyszczeniem i zabezpieczeniem przed błędem."""
    if df_data.empty:
        print(f"ℹ️ Arkusz '{ws_name}' pominięty (brak danych do zapisu).")
        return
    try:
        ...
        print(f"✅ Pomyślnie zaktualizowano arkusz: '{ws_name}' ({len(df_data)} wierszy).")
    except Exception as e_sheet:
        print(f"❌ Błąd zapisu do arkusza '{ws_name}': {e_sheet}")

# ==================================================================================================
# 4. ROZBUDOWANE FUNKCJE MATEMATYCZNE, PROBABILISTYCZNE I BUKMACHERSKIE
# ==================================================================================================

def get_poisson_prob(lam, k, calc_type="exact"):
    """
    Oblicza prawdopodobieństwo w rozkładzie Poissona:
    - exact: P(X = k)
    - under: P(X <= k)
    - over:  P(X > k)
    """
...
def get_poisson_match_prob(lam_h, lam_a, max_val=35):
    """
    Generuje prawdopodobieństwa 1X2 w oparciu o dwuwymiarowy, niezależny rozkład Poissona.
    Zwraca krotkę (P(1), P(X), P(2)).
    """
...
def calc_betbuilder_copula(odds_list, rho=0.65):
    """
    Oblicza kurs łączny dla zdarzeń skorelowanych w formule BetBuilder.
    Wykorzystuje zmodyfikowany wykładnik redukcyjny (kopułę stochastyczną):
    gamma = 1 - rho * (1 - min(Q_joint, Q_next))
    """
    if not odds_list:
        return 1.0
...
def get_weighted_stats(data, target_col, condition_lambda, prior_prob=0.5, alpha=2.0):
    """
    Oblicza ważoną skuteczność historyczną (większa waga dla najświeższych 10 spotkań)
    wraz z wygładzaniem Bayesowskim w przypadku prób poniżej 12 meczów.
    """
    ...
    for i, val in enumerate(valid_values):
        ...
        try:
            is_hit = 1 if condition_lambda(val) else 0
        except Exception:
            is_hit = 0
...
def evaluate_bet(bet_type, r):
    """
    Uniwersalny ewaluator zakładów weryfikujący poprawność predykcji po zakończeniu spotkania.
    Obsługuje rynki 1X2, Over/Under, rzuty rożne, strzały, strzały celne i taśmy łączone (+).
    """
    ...
    if "+" in bet:
        ...
        for p in bet.split("+"):
            ...
            if res == "W OCZEKIWANIU":
                return "W OCZEKIWANIU"
            if res == "PRZEGRANA":
                return "PRZEGRANA"
            if res == "DO RĘCZNEJ KONTROLI":
    ...
    if bet == "1":
        return "WYGRANA" if hg > ag else "PRZEGRANA"
    if bet == "X":
        return "WYGRANA" if hg == ag else "PRZEGRANA"
    if bet == "2":
        return "WYGRANA" if hg < ag else "PRZEGRANA"
    ...
    if bet == "2 (+1.5)":
        return "WYGRANA" if (hg - ag) <= 1 else "PRZEGRANA"
    if bet == "1 (+1.5)":
        return "WYGRANA" if (ag - hg) <= 1 else "PRZEGRANA"
    ...
    if bet.startswith("HU") and hg is not None:
        return "WYGRANA" if hg < float(bet[2:]) else "PRZEGRANA"
    if bet.startswith("AU") and ag is not None:
        return "WYGRANA" if ag < float(bet[2:]) else "PRZEGRANA"
    ...
    if hc is not None and ac is not None:
        tc = hc + ac
        if bet.startswith("C_U"):
            return "WYGRANA" if tc < float(bet[3:]) else "PRZEGRANA"
        if bet.startswith("C_O"):
            return "WYGRANA" if tc > float(bet[3:]) else "PRZEGRANA"
        if bet.startswith("HC_U"):
            return "WYGRANA" if hc < float(bet[4:]) else "PRZEGRANA"
        if bet.startswith("AC_U"):
            return "WYGRANA" if ac < float(bet[4:]) else "PRZEGRANA"
        if bet.startswith("HC_O"):
            return "WYGRANA" if hc > float(bet[4:]) else "PRZEGRANA"
        if bet.startswith("AC_O"):
            return "WYGRANA" if ac > float(bet[4:]) else "PRZEGRANA"
    ...
    if sh is not None and sa is not None:
        t_shots = sh + sa
        if bet == "S_1":
            return "WYGRANA" if sh > sa else "PRZEGRANA"
        if bet == "S_2":
            return "WYGRANA" if sh < sa else "PRZEGRANA"
        if bet == "H_S_O11.5":
            return "WYGRANA" if sh >= 12 else "PRZEGRANA"
        if bet == "H_S_U14.5":
            return "WYGRANA" if sh < 14.5 else "PRZEGRANA"
        if bet == "A_S_U12.5":
            return "WYGRANA" if sa < 12.5 else "PRZEGRANA"
        if bet.startswith("S_U"):
            return "WYGRANA" if t_shots < float(bet[3:]) else "PRZEGRANA"
        if bet.startswith("S_O"):
            return "WYGRANA" if t_shots > float(bet[3:]) else "PRZEGRANA"
    ...
    if sth is not None and sta is not None:
        t_st = sth + sta
        if bet == "ST_1":
            return "WYGRANA" if sth > sta else "PRZEGRANA"
        if bet == "ST_2":
            return "WYGRANA" if sth < sta else "PRZEGRANA"
        if bet == "H_ST_O2.5":
            return "WYGRANA" if sth >= 3 else "PRZEGRANA"
        if bet == "H_ST_U2.5":
            return "WYGRANA" if sth < 2.5 else "PRZEGRANA"
        if bet == "A_ST_U2.5":
            return "WYGRANA" if sta < 2.5 else "PRZEGRANA"
        if bet == "A_ST_U4.5":
            return "WYGRANA" if sta <= 4 else "PRZEGRANA"
        if bet.startswith("ST_U"):
            return "WYGRANA" if t_st < float(bet[4:]) else "PRZEGRANA"
        if bet.startswith("ST_O"):
            return "WYGRANA" if t_st > float(bet[4:]) else "PRZEGRANA"

    return "DO RĘCZNEJ KONTROLI"

# ==================================================================================================
# 5. KOTWICE KURSOWE I DYNAMICZNY MODEL ANCHORINGU
# ==================================================================================================
...
def get_tier_num(tier_str):
    try:
        return int(str(tier_str).replace("Koszyk", "").replace("K", "").strip())
    except Exception:
        return 3
...
# ==================================================================================================
# 6. POBIERANIE DANYCH Z FOOTBALL-DATA.CO.UK (ODPORNOŚĆ NA 503)
# ==================================================================================================
...
def fetch_footballdata_worker(url):
    ...
    max_fd_retries = 3
    for attempt in range(max_fd_retries):
        try:
            ...
            if res.status_code == 200:
                ...
                return pd.DataFrame(), ["Football-Data", u, f"BŁĄD HTTP: Kod {res.status_code}"]
        except requests.exceptions.Timeout:
            time.sleep(1.5)
        except Exception as e_fd:
            time.sleep(1)
            if attempt == max_fd_retries - 1:
                return pd.DataFrame(), ["Football-Data", u, f"BŁĄD SIECI: {str(e_fd)}"]

    return pd.DataFrame(), ["Football-Data", u, "BŁĄD: Serwer niedostępny (Kod 503 / Timeout)"]


def fetch_football_data(raport):
    print("\nPobieram statystyki meczowe z Football-Data (Wielowątkowo)...")
    if not os.path.exists("ligi_footballdata.xlsx"):
        raport.append(["Football-Data", "ligi_footballdata.xlsx", "POMINIĘTO: Brak pliku konfiguracyjnego"])
        return pd.DataFrame()
        
    try:
        urls_fd = pd.read_excel("ligi_footballdata.xlsx")["URL"].dropna().tolist()
    except Exception as e_excel:
        raport.append(["Football-Data", "ligi_footballdata.xlsx", f"BŁĄD Excela: {e_excel}"])
        return pd.DataFrame()
        
    if TEST_MODE:
        urls_fd = urls_fd[:3]
        print(f"🔬 [TEST_MODE] Ograniczono pobieranie Football-Data do {len(urls_fd)} lig.")

    dfs = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS_FOOTBALLDATA) as executor:
        results = executor.map(fetch_footballdata_worker, urls_fd)
    ...
    print(f"✅ Scalono {len(fd_master)} wierszy z bazy Football-Data.")
    return fd_master[existing_cols]

# ==================================================================================================
# 7. WIELOWĄTKOWE POBIERANIE Z BETEXPLORER (ELIMINACJA TIMEOUTU I KODU 'BRAK ODPOWIEDZI')
# ==================================================================================================

try:
    if os.path.exists("ligi.xlsx"):
        urls = pd.read_excel("ligi.xlsx")["URL"].dropna().tolist()
    else:
        urls = []
except Exception as e_ligi:
    print(f"❌ Błąd wczytywania ligi.xlsx: {e_ligi}")
    urls = []

if TEST_MODE and urls:
    urls = urls[:4]
    print(f"🔬 [TEST_MODE] Ograniczono listę URL BetExplorer do {len(urls)} pozycji.")
...
def scrape_be_worker(args):
    ...
    last_err_msg = "Brak odpowiedzi"

    for attempt in range(max_retries):
        if attempt > 0:
            backoff_sleep = random.uniform(3.0, 6.0) * attempt
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                last_err_msg = f"Kod {response.status_code}"
                break
        except requests.exceptions.Timeout:
            last_err_msg = "Timeout (przekroczono czas)"
        except requests.exceptions.ConnectionError:
            last_err_msg = "Błąd połączenia (zerwanie TCP)"
        except Exception as e_req:
            last_err_msg = f"Wyjątek: {type(e_req).__name__}"

    if not success or response is None or response.status_code != 200:
        final_code = response.status_code if (response is not None and hasattr(response, 'status_code')) else last_err_msg
        local_report.append(["BetExplorer", url_clean, f"BŁĄD: {final_code}"])
    ...
    try:
        ...
        if "/fixtures/" in url_clean:
            ...
            for row in rows:
                if not row.find("a", class_="in-match"):
                    continue
                spans = row.find_all("span")
                if len(spans) < 2:
                    continue
        ...
        local_report.append(["BetExplorer", url_clean, f"BŁĄD PARSOWANIA: {e_parse}"])

    return local_data, local_report


all_data = []
print("\nRozpoczynam pobieranie terminarzy i wyników z BetExplorer...")
...
print(f"✅ BetExplorer: Pobrano {len(fixtures_df)} nadchodzących meczów i {len(results_df)} wyników historycznych.")

# ==================================================================================================
# 8. WIELOWĄTKOWE POBIERANIE Z SOCCERSTATS (CURL_CFFI: BRAK 403 I ZERO BŁĘDÓW NAMEERROR)
# ==================================================================================================
...
def scrape_ss_worker(args):
    ...
    try:
        ...
        local_report.append(["SoccerStats", url_ss_clean, f"BŁĄD: {str(e_ss)}"])
...
print("\nRozpoczynam pobieranie z SoccerStats (curl_cffi impersonate Chrome)...")
try:
    if os.path.exists("ligi_soccerstats.xlsx"):
        urls_ss = pd.read_excel("ligi_soccerstats.xlsx")["URL"].dropna().tolist()
        if TEST_MODE:
            urls_ss = urls_ss[:3]
            print(f"🔬 [TEST_MODE] Ograniczono listę URL SoccerStats do {len(urls_ss)} pozycji.")
        ...
        if dane_soccerstats_baza:
            ...
            print(f"✅ SoccerStats: Pomyślnie zintegrowano {len(ss_df)} wyników.")
        else:
            print("ℹ️ SoccerStats: Brak danych z pobranych stron.")
    else:
        scrape_report.append(["SoccerStats", "ligi_soccerstats.xlsx", "POMINIĘTO: Brak pliku Excela"])
        print("ℹ️ Plik ligi_soccerstats.xlsx nie istnieje. Pomięto pobieranie SoccerStats.")
except Exception as e_ss_main:
    scrape_report.append(["SoccerStats", "Główny proces", f"BŁĄD: {e_ss_main}"])
    print(f"❌ Błąd w procesie SoccerStats: {e_ss_main}")

# ==================================================================================================
# 9. MAPOWANIE NAZW, SCALANIE I INTEGRACJA DANYCH
# ==================================================================================================

print("\nScalanie i normalizacja danych ze wszystkich źródeł...")
...
if not fd_df.empty and not results_df.empty:
    ...
    fd_df = fd_df.drop_duplicates(subset=['Date_str', 'HomeTeam', 'AwayTeam'], keep='last').rename(
...
# ==================================================================================================
# 10. BUDOWA ZŁOTEJ STRUKTURY DANYCH (RESULTS & FIXTURES)
# ==================================================================================================
...
if not results_df.empty:
    wszystkie_wymagane = [
        'FTHG', 'FTAG', 'Score', 'HTHG', 'HTAG', 'Gole_Gosp_1H', 'Gole_Gosc_1H',
...
results_clean = (
    results_df[cols_to_extract_res + ['HT_Total', 'Total_Corners', 'Marża']].rename(columns=golden_cols)
    if not results_df.empty else
    pd.DataFrame(columns=list(golden_cols.values()) + ['HT_Total', 'Total_Corners', 'Marża'])
)

cols_fixtures = ['Match_ID', 'Termin', 'Status_Kursów', 'League', 'Date', 'Time', 'Home', 'Away', 'Odd1', 'OddX', 'Odd2', 'Marża']
fixtures_clean = (
    fixtures_df[[c for c in cols_fixtures if c in fixtures_df.columns]].rename(columns={'Odd1': 'Odd_1', 'OddX': 'Odd_X', 'Odd2': 'Odd_2'})
    if not fixtures_df.empty else
    pd.DataFrame(columns=['Match_ID', 'Termin', 'Status_Kursów', 'League', 'Date', 'Time', 'Home', 'Away', 'Odd_1', 'Odd_X', 'Odd_2', 'Marża'])
)

# ==================================================================================================
# 11. GENEROWANIE TABEL LIGOWYCH (6 KOSZYKÓW JAKOŚCIOWYCH)
# ==================================================================================================

print("\nGenerowanie inteligentnych tabel ligowych (6 Koszyków Jakościowych)...")
...
# ==================================================================================================
# 12. WCZYTYWANIE REALNYCH KURSÓW Z BAZY SUPERBET I PEŁNE MAPOWANIE RYNKÓW
# ==================================================================================================
...
if not SKIP_SUPERBET:
    ...
    for j_file in json_files:
        if os.path.exists(j_file):
            try:
                ...
                print(f"⚠️ Błąd wczytywania bazy Superbet ({j_file}): {e_sb}")

    print(f"✅ Łącznie zaindeksowano do bazy Superbet: {len(superbet_baza)} unikalnych spotkań.")
    ...
    for k, v in superbet_baza.items():
        ...
        if gosp_be and gosc_be:
            superbet_fast_lookup[f"{gosp_be}___{gosc_be}"] = v
        if gosp_sb and gosc_sb:
            superbet_fast_lookup[f"{gosp_sb}___{gosc_sb}"] = v
else:
    print("🔬 [SKIP_SUPERBET] Pominięto wczytywanie ofert Superbet w celu przyspieszenia działania.")
...
def get_single_real_odd(match_data, sub_typ, home, away):
    """
    Wyczerpujący parser pojedynczego rynku bukmacherskiego w strukturze JSON Superbet.
    Obsługuje rynki główne (kursy), Multigol (Przedział goli), Strzały 1X2, Połówki oraz Gole drużynowe.
    """
    if not match_data:
        return None
    ...
    # 1. Sprawdzenie bezpośrednie w słowniku 'kursy'
    if sub_k in kursy:
        try:
            return float(kursy[sub_k])
    ...
    home_identifiers = [n for n in [h_sb, h_be, h_orig] if len(n) >= 3]
    away_identifiers = [n for n in [a_sb, a_be, a_orig] if len(n) >= 3]

    # 2. Multigol: MG_1-5, MG_1-6, 1-5, 1-6 itp.
    if sub_k.startswith("MG_") or sub_k in ["1-5", "1-6", "1-4", "2-4", "2-5", "2-6"]:
        range_target = sub_k.replace("MG_", "").strip()
        if range_target in kursy:
            try:
                return float(kursy[range_target])
            except (ValueError, TypeError):
                pass

        for m_name, m_dict in match_data.items():
            if isinstance(m_dict, dict) and any(kw in m_name.lower() for kw in ["przedział goli", "przedzial goli", "zakres goli"]):
                for opt_key, opt_odd in m_dict.items():
                    opt_str = str(opt_key).strip()
                    if opt_str.startswith(range_target) or f"{range_target} |" in opt_str or f"{range_target} goli" in opt_str:
                        try:
                            return float(opt_odd)
                        except (ValueError, TypeError):
                            pass

    # 3. Strzały 1X2: S_1, S_2, ST_1, ST_2
    if sub_k in ["S_1", "S_2", "ST_1", "ST_2"]:
        ...
        for m_name, m_dict in match_data.items():
            ...
            if "strzał" in m_low or "strzal" in m_low:
                if is_target_sot:
                    if any(w in m_low for w in ["na bramkę", "na bramke", "celn", "światło", "swiatlo"]):
                        shots_market = m_dict
                        break
                    elif "najwięcej" in m_low or "najwiecej" in m_low:
                        shots_market = m_dict
                else:
                    if "ogół" in m_low or "ogol" in m_low or ("najwięcej" in m_low and not any(w in m_low for w in ["na bramkę", "na bramke", "celn"])):
                        shots_market = m_dict
                        break
                    elif "najwięcej" in m_low or "najwiecej" in m_low:
                        shots_market = m_dict
        ...
        if shots_market:
            for opt_k, opt_v in shots_market.items():
                ...
                if "remis" in opt_low:
                    continue
                matches_target = any(tn in opt_low for tn in target_names) or any(tn[:5] in opt_low for tn in target_names if len(tn) >= 5)
    ...
    if sub_k.startswith("HT_U") or sub_k.startswith("HT_O"):
        ...
        target_alt = "under" if is_under else "over"

        for m_name, m_dict in match_data.items():
            if not isinstance(m_dict, dict):
                continue
            m_low = m_name.lower()
            if any(p in m_low for p in ["1.połowa", "1. polowa", "1 połowa", "1 polowa", "1. połowa"]) and "liczba goli" in m_low:
                if not any(tn in m_low for tn in (home_identifiers + away_identifiers) if len(tn) >= 4):
                    for opt_k, opt_v in m_dict.items():
                        opt_low = str(opt_k).lower()
                        if (target_kw in opt_low or target_alt in opt_low) and line in opt_low:
                            try:
                                return float(opt_v)
                            except (ValueError, TypeError):
                                pass

    # 5. Gole 2. połowy (2H_U..., 2H_O...)
    if sub_k.startswith("2H_U") or sub_k.startswith("2H_O"):
        is_under = sub_k.startswith("2H_U")
        line = sub_k[4:].strip()
        target_kw = "poniżej" if is_under else "powyżej"
        target_alt = "under" if is_under else "over"
        ...
        for m_name, m_dict in match_data.items():
            if not isinstance(m_dict, dict):
                continue
            m_low = m_name.lower()
            if any(p in m_low for p in ["2.połowa", "2. polowa", "2 połowa", "2 polowa", "2. połowa"]) and "liczba goli" in m_low:
                if not any(tn in m_low for tn in (home_identifiers + away_identifiers) if len(tn) >= 4):
                    for opt_k, opt_v in m_dict.items():
                        opt_low = str(opt_k).lower()
                        if (target_kw in opt_low or target_alt in opt_low) and line in opt_low:
    ...
    # 6. Gole drużynowe (HU, HO, AU, AO)
    if sub_k.startswith(("HU", "HO", "AU", "AO")):
        is_home_target = sub_k.startswith("H")
        is_under = "U" in sub_k[:2]
        line = sub_k[2:].strip()
        target_kw = "poniżej" if is_under else "powyżej"
        target_alt = "under" if is_under else "over"
        ...
        for m_name, m_dict in match_data.items():
            if not isinstance(m_dict, dict):
                continue
            m_low = m_name.lower()
            if any(tn in m_low for tn in target_names) and "liczba goli" in m_low and not any(p in m_low for p in ["1.połowa", "2.połowa", "1. polowa", "2. polowa"]):
                for opt_k, opt_v in m_dict.items():
                    opt_low = str(opt_k).lower()
                    if (target_kw in opt_low or target_alt in opt_low) and line in opt_low:
                        try:
    ...
    # 7. Standardowe Over / Under meczowe (U3.5, O2.5 itp.)
    if (sub_k.startswith("U") or sub_k.startswith("O")) and "_" not in sub_k and not sub_k.startswith(("HC", "AC", "HT", "2H", "HU", "AU")):
        ...
        for m_name, m_dict in match_data.items():
            if not isinstance(m_dict, dict):
                continue
            m_low = m_name.lower()
            if m_low in ["liczba goli", "suma goli", "gole"] or (("liczba goli" in m_low or "suma goli" in m_low) and not any(tn in m_low for tn in (home_identifiers + away_identifiers) if len(tn) >= 4) and not any(p in m_low for p in ["1.połowa", "2.połowa"])):
                for opt_k, opt_v in m_dict.items():
                    opt_low = str(opt_k).lower()
                    if (target_kw in opt_low or target_alt in opt_low) and line in opt_low:
                        try:
    ...
    # 8. Rzuty rożne (C_U..., C_O..., HC_U..., AC_U...)
    if sub_k.startswith(("C_U", "C_O", "HC_U", "HC_O", "AC_U", "AC_O")):
        is_home_corner = sub_k.startswith("HC")
        is_away_corner = sub_k.startswith("AC")
        is_match_corner = sub_k.startswith("C_")
        ...
        for m_name, m_dict in match_data.items():
            ...
            if any(w in m_low for w in ["rzuty rożne", "rzutów rożnych", "rożne", "rozne"]):
                if is_match_corner and not any(tn in m_low for tn in (home_identifiers + away_identifiers) if len(tn) >= 4):
                    for opt_k, opt_v in m_dict.items():
                        opt_low = str(opt_k).lower()
                        if (target_kw in opt_low or target_alt in opt_low) and line in opt_low:
                            try:
                                ...
                                pass
                elif is_home_corner and any(tn in m_low for tn in home_identifiers):
                    for opt_k, opt_v in m_dict.items():
                        opt_low = str(opt_k).lower()
                        if (target_kw in opt_low or target_alt in opt_low) and line in opt_low:
                            try:
                                ...
                                pass
                elif is_away_corner and any(tn in m_low for tn in away_identifiers):
                    for opt_k, opt_v in m_dict.items():
                        opt_low = str(opt_k).lower()
                        if (target_kw in opt_low or target_alt in opt_low) and line in opt_low:
                            try:
...
def get_real_odd(home, away, typ_kod):
    """
    Wyznacza realny kurs z bazy Superbet.
    Dla typów wielokrotnych (BetBuilder / taśmy) odnajduje kursy wszystkich składowych
    i przelicza je zaawansowaną kopułą stochastyczną (Copula).
    """
    ...
    # Złożone konstrukcje BetBuilder (+)
    if "+" in typ_k:
        ...
        for sk in skladniki:
            ...
            if val is not None and val > 1.0:
                kursy_skladowe.append(float(val))
            else:
                return None

        if len(kursy_skladowe) == len(skladniki):
            if "O0.5" in typ_k and ("U5.5" in typ_k or "U6.5" in typ_k) and len(skladniki) == 2:
                ...
                rho_val = 0.22 if any(c in typ_k for c in ["C_U", "HC_", "AC_"]) else 0.45
                return calc_betbuilder_copula(kursy_skladowe, rho=rho_val)
        return None

    # Pojedynczy typ rynkowy
...
# ==================================================================================================
# 13. CENTRALNY GENERATOR PREDYKCJI, MEDALI I WYCENY VALUE
# ==================================================================================================
...
def add_pred(match_id, termin, date, time, league, home, away, engine, typ, szansa, kurs_szac, arg, dyn_anchors=None):
    """
    Dodaje wygenerowaną predykcję z weryfikacją EV, kotwicami rynkowymi i tagowaniem ryzyka:
    - Złoty, Srebrny, Brązowy Typ (wysoka pewność i opłacalność)
    - Real Value (pozytywne EV w oparciu o realny kurs bukmacherski)
    """
    if dyn_anchors is None:
        dyn_anchors = KOTWICE_KURSOWE
    typ_k = str(typ).strip()

    try:
        kurs_matematyczny = float(str(kurs_szac).replace(',', '.'))
    except Exception:
        kurs_matematyczny = 1.05
...
# ==================================================================================================
# 14. GŁÓWNA PĘTLA PREDYKCJI (10 PRECYZYJNYCH SILNIKÓW ANALITYCZNYCH)
# ==================================================================================================
...
for idx, row in fixtures_clean.iterrows():
    ...
    if normal_match or fav_vs_prom_home or fav_vs_prom_away:
        ...
        if fav_vs_prom_home:
            blend_1x = max(blend_1x, 0.89)
        if fav_vs_prom_away:
            blend_x2 = max(blend_x2, 0.86)
        ...
        try:
            ...
            if typ_kod == "1X":
                fair_odd = round((o1 * ox) / (o1 + ox), 2)
            else:
                fair_odd = round((o2 * ox) / (o2 + ox), 2)
        except Exception:
            fair_odd = round(1 / final_prob, 2) if final_prob > 0 else 1.08

        is_logical = True
        if typ_kod == "1X" and t_h > t_a + 2 and not a_is_promoted:
            is_logical = False
        if typ_kod == "X2" and t_a > t_h + 2 and not h_is_promoted:
            is_logical = False
        ...
        if (is_logical or is_fav or fav_vs_prom_home or fav_vs_prom_away) and final_prob >= 0.70 and fair_odd >= 1.04:
            if typ_kod == "1X":
                ...
                if fav_vs_prom_home:
                    arg = f"Gosp ({h_tier_str}) podejmuje beniaminka (Koszyk 6). Punktowanie dom: {round(h_1x_dom_pct*100)}% (1X w {h_1x_c}/{len(h_dom)}). Porażki: [{h_ls_txt}]."
                else:
                    arg = f"Gosp ({h_tier_str}) punktuje dom: {round(h_1x_dom_pct*100)}% (1X w {h_1x_c}/{len(h_dom)}). Porażki: [{h_ls_txt}]. Gość ({a_tier_str}) wygrał {a_win_c}/{len(a_wyj)} (Ogółem: {a_win_tot}/{len(a_tot_all)})."
            else:
                ...
                if fav_vs_prom_away:
                    arg = f"Faworyt ({a_tier_str}) wyjazd z beniaminkiem (Koszyk 6). X2: {round(a_x2_wyj_pct*100)}% ({a_x2_c}/{len(a_wyj)}). Porażki: [{a_ls_txt}]."
                else:
                    arg = f"Gość ({a_tier_str}) punktuje wyjazd: {round(a_x2_wyj_pct*100)}% (X2 w {a_x2_c}/{len(a_wyj)}). Porażki: [{a_ls_txt}]. Gosp ({h_tier_str}) wygrał {h_win_c}/{len(h_dom)} (Ogółem: {h_win_tot}/{len(h_tot_all)})."
    ...
    if len(h_tot_all) >= 10 and len(a_tot_all) >= 10 and len(h_dom) >= 5 and len(a_wyj) >= 5:
        ...
        if h_last_goals == 0 or h_last_goals > 5 or a_last_goals == 0 or a_last_goals > 5:
            ...
            if prob_1_5 >= 0.90 or prob_1_6 >= 0.90:
                typ_kod, pewnosc, hc_cnt, hc_tl, ac_cnt, ac_tl, was_sm = (
                    ("MG_1-5", prob_1_5, h_th, h_tl, a_th, a_tl, (h_sm or a_sm))
                    if prob_1_5 >= 0.90 else
                    ("MG_1-6", prob_1_6, h_th16, h_tl16, a_th16, a_tl16, (h_sm16 or a_sm16))
                )
                est_odd = round(1.0 + (((1/pewnosc) - 1.0) / 1.5), 2)
                arg = f"Regresja po anomalii ({last3_str}). Trafienia D/W: {hc_cnt}/{hc_tl}, {ac_cnt}/{ac_tl}."
                if was_sm:
                    arg += " | ⚠️ Bayes"
    ...
    if len(h_tot_all_c) >= 8 and len(a_tot_all_c) >= 8 and len(h_dom_c) >= 3 and len(a_wyj_c) >= 3:
        ...
        for line in [8.5, 9.5, 10.5, 11.5, 12.5, 13.5, 14.5]:
            if line > max_match - 2:
                ...
                if avg_p >= 0.90:
                    ...
                    c_odds.append(dyn_anchors.get(f"C_U{line}", round(1/(avg_p*0.90), 2)))
        ...
        for line in [4.5, 5.5, 6.5, 7.5, 8.5]:
            if line > max_h - 1:
                ...
                if prob_hc >= 0.92:
                    ...
                    c_odds.append(dyn_anchors.get(f"HC_U{line}", round(1/(prob_hc*0.90), 2)))
        ...
        for line in [3.5, 4.5, 5.5, 6.5, 7.5]:
            if line > max_a - 1:
                ...
                if prob_ac >= 0.92:
                    ...
                    c_odds.append(dyn_anchors.get(f"AC_U{line}", round(1/(prob_ac*0.90), 2)))
        ...
        if len(c_blocks_code) >= 1:
            ...
            est_odd = max(1.05, est_odd)
            uzasadnienie = " | ".join(arg_c)
            if any_smoothed:
                uzasadnienie += " | ⚠️ Bayes"
    ...
    # SILNIK 8: UKRYTA FORMA (Proxy xG)
    for team, is_home in [(home, True), (away, False)]:
        if not valid_shots.empty:
            ...
            if len(t_past) >= 3:
                ...
                pts = sum(
                    3 if (m['FTHG'] > m['FTAG'] if m['Home'] == team else m['FTAG'] > m['FTHG']) else
                    (1 if m['FTHG'] == m['FTAG'] else 0)
                ...
                if st_for >= (st_agg * 1.5) and st_for >= 15 and pts <= 4 and g_for <= 3:
                    ...
                    add_pred(match_id, d_termin, d_date, d_time, league, home, away, "Hidden Form", typ_kod, 80.0, round(est_odd, 2), arg, dyn_anchors)

    # SILNIK 9: ANOMALIE ROŻNYCH
    for team, is_home in [(home, True), (away, False)]:
        t_past_c = valid_corners[(valid_corners['Base_League'] == fixture_base) & ((valid_corners['Home'] == team) | (valid_corners['Away'] == team))].copy()
        if len(t_past_c) >= 8:
            ...
            if season_avg >= 5.5 and last_2_avg <= 3.0:
                ...
                arg = f"Pęknięta seria rożnych. Średnia zespołu: {round(season_avg, 2)}, w 2 ost. meczach tylko: {round(last_2_avg, 2)}. Oczekiwane przełamanie."
                add_pred(match_id, d_termin, d_date, d_time, league, home, away, "Corner Anomalies", typ_kod, 82.0, dyn_anchors.get(typ_kod, round(est_odd, 2)), arg, dyn_anchors)

    # SILNIK 10: ANOMALIE BRAMKOWE
...
# ==================================================================================================
# 15. BACKTESTER, HISTORIA TYPÓW I SYNCHRONIZACJA PREDYKCJI
# ==================================================================================================
...
if not df_all_predictions.empty:
    ...
    for col in ['Termin', 'Wyslij_AKO']:
        if col in nowe_typy_df.columns:
            nowe_typy_df = nowe_typy_df.drop(columns=[col])
    ...
    if not df_historia.empty:
        ...
        if 'Unikalny_Klucz' in do_dodania.columns:
            do_dodania = do_dodania.drop(columns=['Unikalny_Klucz'])

    df_historia = pd.concat([df_historia, do_dodania], ignore_index=True)

# Rozliczanie statusów w Historii Typów
...
# ==================================================================================================
# 16. MODUŁ PORTFELA REALNEGO (KUPONY AKO - TYLKO DLA RĘCZNIE ZAGRANYCH KUPONÓW)
# ==================================================================================================
...
try:
    ...
    if len(ako_dane) > 0:
        df_ako = pd.DataFrame(ako_dane[1:], columns=ako_dane[0])
    else:
...
# Przetwarzanie portfela wyłącznie dla kuponów posiadających nadany Kupon_ID
if not df_historia.empty:
    kupony_istniejace = df_historia[df_historia['Kupon_ID'].astype(str).str.strip() != ""]
    nowe_ako_list = []

    if not kupony_istniejace.empty:
        grupy_ako = kupony_istniejace.groupby('Kupon_ID')
        for k_id, group in grupy_ako:
            ...
            if "PRZEGRANA" in statusy:
                status_ako = "PRZEGRANA"
            elif "W OCZEKIWANIU" in statusy:
                status_ako = "W OCZEKIWANIU"
            elif all(s == "WYGRANA" for s in statusy):
                status_ako = "WYGRANA"
            else:
                status_ako = "ZWRÓCONY"
            ...
            if status_ako == "WYGRANA":
                profit = round(wygrana_brutto - stawka, 2)
            elif status_ako == "PRZEGRANA":
                profit = -stawka
            else:
                profit = 0.0

            nowe_ako_list.append([
                k_id, data_zawarcia, mecze_skrot, liczba_zdarzen, kurs_ako,
                stawka, jednostki_str, status_ako, wygrana_brutto, profit, wyslij_pod, tel_status
            ])

    if nowe_ako_list:
        df_ako = pd.DataFrame(nowe_ako_list, columns=cols_ako).sort_values(by="Data_Zawarcia", ascending=False)
    else:
        df_ako = pd.DataFrame(columns=cols_ako)

if not df_historia.empty:
    df_historia['Data_Sort'] = pd.to_datetime(
        df_historia['Data'].astype(str) + ' ' + df_historia['Godzina'].astype(str).replace('', '00:00').replace('-', '00:00'),
        errors='coerce'
    )
...
if not df_all_predictions.empty:
    df_all_predictions['Data_Sort'] = pd.to_datetime(
        df_all_predictions['Data'].astype(str) + ' ' + df_all_predictions['Godzina'].astype(str).replace('', '00:00').replace('-', '00:00'),
        errors='coerce'
    )
    now_time = datetime.now()
    df_all_predictions = df_all_predictions[df_all_predictions['Data_Sort'] >= now_time - timedelta(hours=3)]
    df_all_predictions = df_all_predictions.sort_values(by=["Data_Sort", "Szansa"], ascending=[True, False]).drop(
        columns=['Data_Sort', 'Unikalny_Klucz'], errors='ignore'
    )

# ==================================================================================================
# 17. GENEROWANIE INTELIGENTNEGO WIDOKU TOP WYBORY (RÓŻNORODNOŚĆ SILNIKÓW & JAKOŚĆ)
# ==================================================================================================

top_wybory_df = pd.DataFrame()

if not df_all_predictions.empty:
    print("\nGenerowanie zoptymalizowanego widoku Top Wybory (Różnorodność silników)...")
    try:
        spreadsheet.worksheet("Top_Wybory")
    except gspread.exceptions.WorksheetNotFound:
    ...
    if not active_pred.empty:
        ...
        top_wybory_list = []
        unikalne_silniki = active_pred['Engine'].unique()

        for eng in unikalne_silniki:
            df_eng = active_pred[active_pred['Engine'] == eng].sort_values(
                by=['Szansa_Num', 'Kurs_Num'],
                ascending=[False, False]
            )

            picks_90 = df_eng[df_eng['Szansa_Num'] >= 90.0]
            if len(picks_90) >= 20:
                selected_eng = picks_90
            else:
                selected_eng = df_eng.head(20)

            top_wybory_list.append(selected_eng)

        if top_wybory_list:
            top_wybory_df = pd.concat(top_wybory_list, ignore_index=True)
            top_wybory_df = top_wybory_df.drop_duplicates(subset=['Match_ID', 'Engine', 'Typ'])
            ...
            cols_wybory = [
                ...
                "Engine", "Typ", "Szansa", "Kurs_Szac", "Kurs_Realny", "Argumentacja"
            ]
            top_wybory_df = top_wybory_df[[c for c in cols_wybory if c in top_wybory_df.columns]]

# ==================================================================================================
# 18. FINALNY ZAPIS ZSYNCHRONIZOWANYCH DANYCH DO GOOGLE SHEETS
# ==================================================================================================

all_sheets = [
    "Summary", "Fixtures", "Results", "League_Tables",
    "Historia_Typow", "All_Predictions", "Top_Wybory", "Kupony_AKO"
]
...
for sheet_name in all_sheets:
    try:
        ...
        spreadsheet.add_worksheet(title=sheet_name, rows=1000, cols=30)

print("\nRozpoczynam finalny zapis zintegrowanych danych do Google Sheets...")
...
# Zapis raportu i podsumowania do Summary
summary_data = [
    ["==== PODSUMOWANIE OGÓLNE ====", "", ""],
...
print("1. Poprawiono mapowanie Superbet: Multigol (MG_1-5, MG_1-6), Strzały (S_1, ST_1) i BetBuilder Pro.")
print("2. Wyeliminowano błędy składniowe (SyntaxError) w kodzie.")
print("3. Zachowano inteligentną selekcję Top_Wybory i usunięto automatyczne zaśmiecanie bazy AKO.")
print("=" * 90)
