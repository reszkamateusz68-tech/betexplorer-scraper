...
# PRZYWRÓCONA: Zliczanie koszyków przeciwników dla lepszej argumentacji
...
# ZAKTUALIZOWANA, PRECYZYJNA BAZA KOTWIC ZGODNIE Z DOKUMENTACJĄ
KOTWICE_KURSOWE = {
    'O0.5': 1.03, 'U3.5': 1.31, 'U4.5': 1.10, 'U5.5': 1.03, 'U6.5': 1.01,
...
def add_pred(match_id, termin, date, time, league, home, away, engine, typ, kurs_rynek, szansa, kurs_szac, arg):
    ...
    if engine == "BetBuilder Pro":
        ...
        # Dla bramek używamy rho=0.85 (silna korelacja), dla rożnych rho=0.22/0.65
        rho_val = 0.22 if any(c in typ_k for c in ["C_U", "HC_", "AC_"]) else 0.85
    ...
    elif prob_decimal >= 0.95 and 1.10 <= kurs_docelowy < 1.20:
        risk_tag = "🥈 SILVER PICK"
...
for idx, row in fixtures_clean.iterrows():
    ...
    if len(h_tot_all) >= 10 and len(a_tot_all) >= 10 and len(h_dom) >= 5 and len(a_wyj) >= 5:
        ...
        for line in [2.5, 3.5, 4.5, 5.5, 6.5]:
            ...
            if avg_prob_u >= 0.70:
                ...
                arg = f"U{line} | Ważone szanse: {round(avg_prob_u*100)}%. Gosp ({h_tier}): {h_stat} | Gość ({a_tier}): {a_stat}. Ogółem: Gosp {ht_th}/{ht_tl}, Gość {at_th}/{at_tl}."
        ...
        for line in [0.5, 1.5, 2.5]:
            ...
            if avg_prob_o >= 0.70: 
                h_stat = get_tier_stats(h_dom, True, league, team_tiers, 'Total_Goals', lambda x: pd.notna(x) and x > line)
                a_stat = get_tier_stats(a_wyj, False, league, team_tiers, 'Total_Goals', lambda x: pd.notna(x) and x > line)
                arg = f"O{line} | Ważone szanse: {round(avg_prob_o*100)}%. Gosp ({h_tier}): {h_stat} | Gość ({a_tier}): {a_stat}. Ogółem: Gosp {ht_th}/{ht_tl}, Gość {at_th}/{at_tl}."
                if h_sm or a_sm: arg += " | ⚠️ Wygładzenie Bayesowskie"
    ...
    # ----------------------------------------------------
    # 6c. BETBUILDER PRO (Dynamiczny Konstruktor Value)
    # ----------------------------------------------------
    PROG_OVER = 0.88
    PROG_UNDER = 0.88
    MIN_BLOKOW = 2
    ...
    if len(h_tot_all) >= 10 and len(a_tot_all) >= 10 and len(h_dom) >= 5 and len(a_wyj) >= 5:
        ...
        if prob_o05 >= PROG_OVER:
            h_stat = get_tier_stats(h_dom, True, league, team_tiers, 'Total_Goals', lambda x: pd.notna(x) and x >= 1)
            a_stat = get_tier_stats(a_wyj, False, league, team_tiers, 'Total_Goals', lambda x: pd.notna(x) and x >= 1)
            builder_blocks_code.append("O0.5")
            block_probabilities.append(prob_o05)
            arg_blocks.append(f"O0.5 (Gosp: {h_stat}, Gość: {a_stat})")
        ...
        for line in [3.5, 4.5, 5.5, 6.5]:
            ...
            if prob_u >= PROG_UNDER:
                ...
                arg_blocks.append(f"U{line} (Gosp: {h_stat}, Gość: {a_stat})")
                break 
        ...
        for line in [1.5, 2.5, 3.5, 4.5]:
            ...
            if prob_u_1h >= PROG_UNDER:
                h_stat = get_tier_stats(h_dom, True, league, team_tiers, 'HT_Total', lambda x: pd.notna(x) and x < line)
                a_stat = get_tier_stats(a_wyj, False, league, team_tiers, 'HT_Total', lambda x: pd.notna(x) and x < line)
                builder_blocks_code.append(f"HT_U{line}")
                block_probabilities.append(prob_u_1h)
                arg_blocks.append(f"HT_U{line} (Gosp: {h_stat}, Gość: {a_stat})")
                break
        ...
        for line in [3.5, 4.5]:
             ...
             if prob_u_2h >= PROG_UNDER:
                h_stat = get_tier_stats(h_dom, True, league, team_tiers, '2H_Total', lambda x: pd.notna(x) and x < line)
                a_stat = get_tier_stats(a_wyj, False, league, team_tiers, '2H_Total', lambda x: pd.notna(x) and x < line)
                builder_blocks_code.append(f"2H_U{line}")
                block_probabilities.append(prob_u_2h)
                arg_blocks.append(f"2H_U{line} (Gosp: {h_stat}, Gość: {a_stat})")
                break
                
        for t_code, t_df, t_goals_col, opp_code, opp_df, opp_goals_col in [("H", h_dom, "FTHG", "A", a_wyj, "FTAG")]:
            for line in [2.5, 3.5, 4.5]:
                prob_t_u_h, th, tl, hsm = get_weighted_stats(t_df, t_goals_col, lambda x: pd.notna(x) and x < line, prior_prob=0.85)
                prob_t_u_a, oh, ol, asm = get_weighted_stats(opp_df, opp_goals_col, lambda x: pd.notna(x) and x < line, prior_prob=0.85)
                prob_t_u = (prob_t_u_h + prob_t_u_a) / 2
                if hsm or asm: any_smoothed = True
                if prob_t_u >= PROG_UNDER:
                    t_stat = get_tier_stats(t_df, True, league, team_tiers, t_goals_col, lambda x: pd.notna(x) and x < line)
                    o_stat = get_tier_stats(opp_df, False, league, team_tiers, opp_goals_col, lambda x: pd.notna(x) and x < line)
                    builder_blocks_code.append(f"{t_code}U{line}")
                    block_probabilities.append(prob_t_u)
                    arg_blocks.append(f"{t_code}U{line} (Gosp zdobywa: {t_stat}, Gość traci: {o_stat})")
                    break
        
        for t_code, t_df, t_goals_col, opp_code, opp_df, opp_goals_col in [("A", a_wyj, "FTAG", "H", h_dom, "FTHG")]:
             for line in [2.5, 3.5, 4.5]:
                prob_t_u_a, th, tl, asm = get_weighted_stats(t_df, t_goals_col, lambda x: pd.notna(x) and x < line, prior_prob=0.85)
                prob_t_u_h, oh, ol, hsm = get_weighted_stats(opp_df, opp_goals_col, lambda x: pd.notna(x) and x < line, prior_prob=0.85)
                prob_t_u = (prob_t_u_a + prob_t_u_h) / 2
                if hsm or asm: any_smoothed = True
                if prob_t_u >= PROG_UNDER:
                    t_stat = get_tier_stats(t_df, False, league, team_tiers, t_goals_col, lambda x: pd.notna(x) and x < line)
                    o_stat = get_tier_stats(opp_df, True, league, team_tiers, opp_goals_col, lambda x: pd.notna(x) and x < line)
                    builder_blocks_code.append(f"{t_code}U{line}")
                    block_probabilities.append(prob_t_u)
                    arg_blocks.append(f"{t_code}U{line} (Gość zdobywa: {t_stat}, Gosp traci: {o_stat})")
                    break

        if len(builder_blocks_code) >= MIN_BLOKOW:
            final_builder_safety = round(np.mean(block_probabilities) * 100, 1)
            estimated_bb_odd = calc_betbuilder_copula([KOTWICE_KURSOWE.get(c, 1.05) for c in builder_blocks_code], rho=0.85)
    ...
    if len(h_tot_all) >= 10 and len(a_tot_all) >= 10 and len(h_dom) >= 5 and len(a_wyj) >= 5:
        ...
        if h_last_goals == 0 or h_last_goals > 5 or a_last_goals == 0 or a_last_goals > 5:
            ...
            if prob_1_5 >= 0.90 or prob_1_6 >= 0.90:
                ...
                typ_kod, pewnosc, was_sm = ("O0.5+U5.5", prob_1_5, (h_sm or a_sm)) if prob_1_5 >= 0.90 else ("O0.5+U6.5", prob_1_6, (h_sm16 or a_sm16))
                est_odd = KOTWICE_KURSOWE.get(typ_kod, 1.09)
                ...
                arg = f"Regresja po anomalii (Wyniki Gosp ost. 3: {h_scores} | Gość ost. 3: {a_scores}). Gosp: {h_stat}. Gość: {a_stat}."
    ...
    if len(h_tot_all_c) >= 8 and len(a_tot_all_c) >= 8 and len(h_dom_c) >= 3 and len(a_wyj_c) >= 3:
        ...
        for line in [8.5, 9.5, 10.5, 11.5, 12.5, 13.5, 14.5]:
            if line > max_match - 2:
                ...
                if avg_p >= 0.90:
                    if h_sm or a_sm: any_smoothed = True
                    c_blocks_code.append(f"C_U{line}"); c_probs.append(avg_p); c_odds.append(KOTWICE_KURSOWE.get(f"C_U{line}", 1.05))
        ...
        for line in [4.5, 5.5, 6.5, 7.5, 8.5]:
            if line > max_h - 1:
                ...
                if prob_hc >= 0.92:
                    ...
                    c_blocks_code.append(f"HC_U{line}"); c_probs.append(prob_hc); c_odds.append(KOTWICE_KURSOWE.get(f"HC_U{line}", 1.05))
        ...
        for line in [3.5, 4.5, 5.5, 6.5, 7.5]:
            if line > max_a - 1:
                ...
                if prob_ac >= 0.92:
                    ...
                    c_blocks_code.append(f"AC_U{line}"); c_probs.append(prob_ac); c_odds.append(KOTWICE_KURSOWE.get(f"AC_U{line}", 1.05))
    ...
    if not valid_shots.empty:
        ...
        if len(h_dom_s) >= 2 and len(a_wyj_s) >= 2 and len(h_tot_all_s) >= 2 and len(a_tot_all_s) >= 2:
            ...
            if prob_h_st > 0.80:
                ...
                add_pred(match_id, d_termin, d_date, d_time, league, home, away, "Shots Pro", "ST_1", "", round(prob_h_st*100, 1), round(est_odd_st, 2), arg_st)
    ...
    for team, is_home in [(home, True), (away, False)]:
        ...
        if len(t_past) >= 8:
            ...
            if season_avg >= 5.5 and last_2_avg <= 3.0:
                ...
                est_odd = KOTWICE_KURSOWE.get(typ_kod, 1.44)
    ...
    if len(t_past) >= 10:
        ...
        if season_avg <= 2.8 and last_2_avg >= 4.5:
            est_odd = KOTWICE_KURSOWE.get("U3.5", 1.31)
            ...
            est_odd = KOTWICE_KURSOWE.get("O1.5", 1.25)
