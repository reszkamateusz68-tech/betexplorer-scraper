...
def fetch_superbet_odds_for_fixtures(fixtures_df, raport):
    ...
    for d_str in [today_str, tomorrow_str, day_after_str]:
        try:
            ...
            raport.append(["Superbet API", "Błąd żądania sieciowego", str(e)])

    if not events: 
        raport.append(["Superbet API", "Brak danych", "BŁĄD: Pusta odpowiedź z API Superbet"])
        return {}

    # --- OPTYMALIZACJA WĄSKIEGO GARDŁA (Twarde Dopasowanie Dual-Match) ---
    be_matches = []
    for _, r in fixtures_df.iterrows():
        # Pobieramy pierwsze 5 znaków po usunięciu popularnych przedrostków
    ...
    for ev in events:
        ...
        for h_be, a_be in be_matches:
            # Dopasowanie musi zawierać OBA skróty, by wykluczyć błędne żądania
    ...
    # Zmniejszona liczba wątków + twardy filtr gwarantują szybkie wykonanie bez limitów
    ...
    raport.append(["Superbet API", "Zoptymalizowany Fetch JSON", f"OK (Zmapowano: {len(superbet_db)} meczów)"])
...
def get_real_odd(home, away, typ_kod, fallback_kurs):
    ...
    Zabezpieczona przed fałszywymi trafieniami dzięki twardemu czyszczeniu nazw drużyn.
    """
...
for idx, row in fixtures_clean.iterrows():
    ...
    if not valid_shots.empty:
        ...
        if len(h_dom_s) >= 2 and len(a_wyj_s) >= 2 and len(h_tot_all_s) >= 2 and len(a_tot_all_s) >= 2:
            ...
            a_tot_all_s['Opp_ST'] = np.where(a_tot_all_s['Home'] == away, a_tot_all_s['Shots_A'], a_tot_all_s['Shots_H'])
...
safe_batch_update("H2H_Mecze", df_h2h)
...
print("Zoptymalizowano pobieranie Superbet - wyeliminowano rate limiting i nieskończoną pętlę.")
print("=" * 60)
