...
target_day = now.date()
end_day = target_day + timedelta(days=5)

start_iso = f"{target_day}T00:00:00.000Z"
end_iso = f"{end_day}T00:00:00.000Z"
...
print(f"1. Pobieram kalendarz meczów na 5 DNI ({start_iso[:10]} do {end_iso[:10]})...")
...
print(f"2. Znaleziono {total_events} spotkań. Pobieram pełną ofertę rynków (40 wątków)...")
...
if os.path.exists("slownik_druzyn.json"):
    try:
        with open("slownik_druzyn.json", "r", encoding="utf-8") as f:
            slownik = json.load(f)
            map_be = slownik.get("BetExplorer_To_Superbet", {})
            mapowanie_sb = {v.strip().lower(): k for k, v in map_be.items() if v.strip() != ""}
    except Exception:
        pass
...
def fetch_single_event(event_id):
    ...
    try:
        # Pobieranie strumieniowe z natychmiastowym przerwaniem socketu
        r = session.get(detail_url, stream=True, timeout=6)
        raw_match = None
        ...
        try:
            ...
            # Wymuszenie natychmiastowego zamknięcia gniazda TCP
            r.raw.close()
            r.close()
...
with ThreadPoolExecutor(max_workers=40) as executor:
    ...
    for future in as_completed(futures):
        ...
        if rows:
            ...
            print(f"[{completed_count:03d}/{total_events}] Pobrano ({len(rows)} kursów): {m_name}")
        else:
            print(f"[{completed_count:03d}/{total_events}] Brak rynków dla: {m_name}")

elapsed = round(time.time() - start_time, 2)

if all_rows:
    df = pd.DataFrame(all_rows)
    df.drop(columns=["Kurs_Float"]).to_csv("superbet_baza_kursow_5dni.csv", sep=";", index=False, encoding="utf-8-sig")

    print("\n3. Budowanie struktury bazy JSON...")
    superbet_db = {}
    ...
    for row in all_rows:
        ...
        # 5. Gole 1. połowa
        elif "1. połowa - liczba goli" in rynek_l or "1.połowa - liczba goli" in rynek_l:
            m_line = re.search(r'(\d+(?:\.\d+)?)', typ_l)
            if m_line:
                line_val = m_line.group(1)
                if "poniżej" in typ_l or typ_l.startswith("-"):
    ...
    with open("superbet_baza_5dni.json", "w", encoding="utf-8") as f:
        json.dump(superbet_db, f, ensure_ascii=False, indent=2)

    print(f"\nSUKCES! Baza Superbet (5 DNI) pobrana w {elapsed} s.")