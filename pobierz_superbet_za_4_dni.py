...
target_day = now.date() + timedelta(days=4)
...
print(f"1. Pobieram kalendarz meczów na POPOPOJUTRZE ({start_iso[:10]})...")
...
print(f"2. Znaleziono {total_events} meczów na popopojutrze. Pobieram pełną ofertę rynków (16 wątków)...")

if not total_events:
    print("Brak meczów na popopojutrze w ofercie.")
    exit()
...
if all_rows:
    ...
    df_csv.to_csv("superbet_baza_kursow_popopojutrze.csv", sep=";", index=False, encoding="utf-8-sig")
    ...
    with open("superbet_baza_popopojutrze.json", "w", encoding="utf-8") as f:
        json.dump(superbet_db, f, ensure_ascii=False, indent=2)

    print(f"SUKCES! Baza Superbet (POPOPOJUTRZE) została zaktualizowana w {elapsed} s.")