...
print(f"1. Pobieram kalendarz meczów na DZIŚ ({start_iso[:10]})...")
...
print(f"2. Znaleziono {total_events} meczów na dziś. Pobieram pełną ofertę rynków (16 wątków)...")

if not total_events:
    print("Brak meczów na dziś w ofercie.")
    exit()
...
if all_rows:
    ...
    df_csv.to_csv("superbet_baza_kursow_dzis.csv", sep=";", index=False, encoding="utf-8-sig")
    ...
    for row in all_rows:
        ...
        # 4. Gole meczowe (Tylko oficjalny rynek "Liczba goli")
    ...
    with open("superbet_baza_dzis.json", "w", encoding="utf-8") as f:
        json.dump(superbet_db, f, ensure_ascii=False, indent=2)

    print(f"SUKCES! Baza Superbet została zaktualizowana w {elapsed} s.")