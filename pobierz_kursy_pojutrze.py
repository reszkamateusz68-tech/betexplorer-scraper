...
# ==========================================================
# 1. POBIERANIE KALENDARZA (POJUTRZE)
# ==========================================================
now = datetime.now(timezone.utc)
day_after = now.date() + timedelta(days=2)
day_after_3 = day_after + timedelta(days=1)

start_iso = f"{day_after}T00:00:00.000Z"
end_iso = f"{day_after_3}T00:00:00.000Z"
...
print(f"1. Pobieram kalendarz meczów na POJUTRZE ({start_iso[:10]})...")
...
print(f"2. Znaleziono {total_events} meczów na pojutrze. Pobieram pełną ofertę kursów (16 wątków)...")

if not total_events:
    print("Brak meczów na pojutrze w ofercie.")
    exit()
...
if all_rows:
    ...
    csv_file = "superbet_baza_kursow_pojutrze.csv"
    ...
    json_file = "superbet_baza_pojutrze.json"
    with open(json_file, "w", encoding="utf-8") as f:
        json.dump(superbet_db, f, ensure_ascii=False, indent=2)

    print(f"\nSUKCES! Pomyślnie zapisano kompletne dane na POJUTRZE w {elapsed} s.")
