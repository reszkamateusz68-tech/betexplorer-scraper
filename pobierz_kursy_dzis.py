...
print(f"2. Znaleziono {total_events} meczów na dziś. Pobieram pełną ofertę kursów (16 wątków)...")
...
if all_rows:
    ...
    csv_file = "superbet_baza_kursow_dzis.csv"
    ...
    for row in all_rows:
        ...
        # A) ZAPIS 100% RYNKÓW (Hierarchia z CSV)
    ...
    json_file = "superbet_baza_dzis.json"
    with open(json_file, "w", encoding="utf-8") as f:
        json.dump(superbet_db, f, ensure_ascii=False, indent=2)

    print(f"\nSUKCES! Pomyślnie zapisano kompletne dane w {elapsed} s.")
    print(f"Plik CSV: {csv_file}")
    print(f"Plik JSON: {json_file} (zawiera 100% rynków z CSV)")
