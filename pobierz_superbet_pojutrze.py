...
target_day = now.date() + timedelta(days=2)
...
print(f"2. Znaleziono {total_events} meczów na pojutrze. Pobieram pełną ofertę rynków (16 wątków)...")
...
if all_rows:
    ...
    df_csv.to_csv("superbet_baza_kursow_pojutrze.csv", sep=";", index=False, encoding="utf-8-sig")
    ...
    with open("superbet_baza_pojutrze.json", "w", encoding="utf-8") as f:
        json.dump(superbet_db, f, ensure_ascii=False, indent=2)

    print(f"SUKCES! Baza Superbet (POJUTRZE) została zaktualizowana w {elapsed} s.")
